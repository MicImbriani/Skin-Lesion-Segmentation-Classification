...
def double_convolution(in_ch, out_ch, mid_ch=None, optimised=False):
    ...
    if optimised:
        convolution = nn.Sequential(nn.Conv2d(in_ch, mid_ch, kernel_size=3, padding=1),
...
def crop_img(input_tensor, target_tensor):
    ...
    diffY = target_tensor.size()[2] - input_tensor.size()[2]
    diffX = target_tensor.size()[3] - input_tensor.size()[3]

    x1 = F.pad(input_tensor, [diffX // 2, diffX - diffX // 2,
